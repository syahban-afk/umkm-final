                ...
                <div class="p-6">
                    <div class="flex flex-col md:flex-row md:justify-between md:items-center">
                        <div class="mb-4 md:mb-0">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white">Pesanan #{{ $order->id }}
                            </h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Tanggal:
                        ...
                        <div class="flex items-center">
                            ...
                            @if ($order->status === 'pending')
                                <form action="{{ route('orders.cancel', $order->id) }}" method="POST" class="ml-4">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                                    ...
                                                    <div class="flex items-center">
                                                        ...
                                                        <div class="ml-4">
                                                            ...
                                                            <div class="text-sm text-gray-500 dark:text-gray-400">
                                                                {{ $detail->product->category->name ?? 'Tanpa Kategori' }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
                ...
                <div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <span class="text-gray-900 dark:text-white font-medium">
                                        Rp {{ number_format($subtotalBeforeDiscount ?? 0, 0, ',', '.') }}
                                    </span>
                                </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                        <div class="p-6">
                            ...
                            @if (
                                $order->payment->payment_method === 'transfer' &&
                                    $order->payment->status !== 'paid' &&
                                    $order->status !== 'cancelled')
                                <div class="mt-4 p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
                                    <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2">Instruksi
                                        Pembayaran:</h4>
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">Silakan transfer ke
                                        rekening berikut:</p>
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-sm text-gray-900 dark:text-white">Atas Nama: UMKM Marketplace
                                        </p>
                                    </div>
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">Jumlah: Rp
                        ...
                        <div class="p-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div>
                                    ...
                                    <span
                                        class="text-gray-900 dark:text-white">{{ Auth::user()->customer->address ?? 'Alamat tidak tersedia' }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600 dark:text-gray-400 block">Telepon:</span>
                                    <span
                                        class="text-gray-900 dark:text-white">{{ Auth::user()->customer->phone ?? 'Telepon tidak tersedia' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
