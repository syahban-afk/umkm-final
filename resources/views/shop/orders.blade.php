                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Pembayaran</th>
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach($orders as $order)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-500 dark:text-gray-400">{{ $order->order_date->format('d M Y') }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @if($order->payment)
                                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                                        @if($order->payment->status == 'pending') bg-yellow-100 text-yellow-800 @endif
                                                        @if($order->payment->status == 'paid') bg-green-100 text-green-800 @endif
                                                        @if($order->payment->status == 'failed') bg-red-100 text-red-800 @endif
                                                    ">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <a href="{{ route('orders.show', $order->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 mr-3">Detail</a>

                                                @if(($order->payment && $order->payment->status == 'pending') || !$order->payment)
                                                    <a href="{{ route('orders.payment', $order->id) }}" class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300">Upload Bukti</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
            ...
            @else
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-center">
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Anda belum memiliki riwayat pesanan.</p>
                        <div class="mt-6">
