        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="flex flex-col md:flex-row md:justify-between md:items-center">
                        <div class="mb-4 md:mb-0">
                            ...
                            <div class="mt-2 flex items-center">
                                ...
                                <span class="ml-2 text-sm text-gray-500 dark:text-gray-400">Tanggal Pesanan: {{ $order->order_date->format('d M Y') }}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-2 flex items-center">
                                ...
                                @if($order->payment)
                                    <span class="ml-2 text-sm text-gray-500 dark:text-gray-400">Metode: {{ ucfirst($order->payment->payment_method) }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tfoot class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">Rp {{ number_format($order->total_amount - ($order->delivery ? $order->delivery->shipping_cost : 0), 0, ',', '.') }}</td>
                                </tr>
                                <tr>
                                    <td colspan="3" class="px-6 py-4 text-right text-sm font-medium text-gray-900 dark:text-white">Biaya Pengiriman:</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">Rp {{ number_format($order->delivery ? $order->delivery->shipping_cost : 0, 0, ',', '.') }}</td>
                                </tr>
                                <tr>
                                    <td colspan="3" class="px-6 py-4 text-right text-sm font-medium text-gray-900 dark:text-white">Total:</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $order->customer->name }}</p>
                        ...
                        <div>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                @if($order->delivery)
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Kurir: <span class="font-medium text-gray-900 dark:text-white">{{ $order->delivery->courier_name }}</span></p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">No. Resi: <span class="font-medium text-gray-900 dark:text-white">{{ $order->delivery->tracking_number }}</span></p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Status:
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            ...
                                            @if($order->delivery->status == 'shipped') bg-blue-100 text-blue-800 @endif
                                    ...
                                    @if($order->delivery->delivery_date)
                                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">Tanggal Pengiriman: <span class="font-medium text-gray-900 dark:text-white">{{ $order->delivery->delivery_date->format('d M Y') }}</span></p>
                                    @endif
                                @else
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Informasi pengiriman belum tersedia.</p>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex justify-end space-x-4">
                @if(($order->payment && $order->payment->status == 'pending') || !$order->payment)
                    <a href="{{ route('orders.payment', $order->id) }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                ...
                @if($order->status == 'pending')
                    <form action="{{ route('orders.cancel', $order->id) }}" method="POST" class="inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:bg-red-700 active:bg-red-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150" onclick="return confirm('Apakah Anda yakin ingin membatalkan pesanan ini?')">
                            Batalkan Pesanan
                        </button>
                    </form>
                @endif
                ...
                @if($order->status == 'completed')
                    <a href="{{ route('reviews.create', ['order_id' => $order->id]) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                        Beri Ulasan
                    </a>
                @endif
            </div>
